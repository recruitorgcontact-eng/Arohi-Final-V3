...
export default function PricingPage({
  ...
  return (
    <div className={`w-full max-w-7xl mx-auto ${embedMode ? 'p-2 sm:p-4' : 'px-4 py-8 md:py-12'} space-y-10 text-white font-sans`}>
      
      {/* PAGE HEADER */}
      <div className="text-center space-y-4 max-w-3xl mx-auto">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-gradient-to-r from-purple-900/60 via-fuchsia-900/60 to-indigo-900/60 border border-purple-500/40 shadow-lg text-xs font-black uppercase tracking-widest text-amber-300">
          <Crown className="w-4 h-4 text-amber-400 animate-bounce" />
          Arohi AI Official Subscription Plans
        </div>

        <h1 className="text-3xl md:text-5xl font-black tracking-tight text-white leading-tight">
          Flexible Pricing from <span className="bg-clip-text text-transparent bg-gradient-to-r from-amber-300 via-yellow-200 to-amber-400">₹399 to ₹4,999</span> / Month
        </h1>
        ...
        {/* Highlighted Banner */}
        <div className="inline-flex flex-wrap items-center justify-center gap-3 bg-emerald-950/80 border border-emerald-500/50 px-5 py-2.5 rounded-2xl shadow-xl text-xs font-extrabold text-emerald-300">
          ...
          <span className="text-slate-200 font-bold">No Hidden Charges • Instant UPI Activation</span>
        </div>
      </div>
        ...
        {PRICING_TIERS.map((tier, idx) => {
          ...
          return (
            <div
              ...
              className={`rounded-[2rem] p-5 text-left transition-all duration-300 relative flex flex-col justify-between border-2 cursor-pointer ${
                isSelected || isPopular
                  ? 'bg-gradient-to-b from-[#1c123d] via-[#140b2e] to-[#0d0722] border-purple-400/80 shadow-[0_0_30px_rgba(168,85,247,0.35)] scale-[1.02]'
                  : 'bg-[#120b29]/80 border-[#261b54] hover:border-purple-500/50 hover:bg-[#180f36]'
              }`}
            >
              {/* Top Badges */}
              {isPopular && (
              ...
              <div className="space-y-4">
                {/* Header info */}
                <div className="space-y-1.5 border-b border-purple-900/40 pb-3">
                ...
                <div className="space-y-2 bg-gradient-to-br from-violet-950/70 to-purple-950/50 p-3 rounded-2xl border border-violet-500/30">
                  <div className="text-[9px] font-black uppercase tracking-wider text-amber-300 flex items-center justify-between">
                    ...
                    <span className="text-emerald-400 font-extrabold">Full Access</span>
                  </div>

                  {/* UNLIMITED CHAT */}
                  <div className="bg-emerald-950/80 border border-emerald-500/50 p-1.5 rounded-xl flex items-center gap-1.5 text-[9.5px] font-black text-emerald-300">
                    <Sparkles className="w-3.5 h-3.5 text-emerald-400 shrink-0 animate-pulse" />
                    <span>✨ Unlimited AI Chat</span>
                  </div>

                  {/* AI Tokens */}
                  <div className="bg-purple-900/40 border border-purple-500/25 p-1.5 rounded-xl flex items-center gap-1.5 text-[10px] font-extrabold text-cyan-200">
                    <Cpu className="w-3.5 h-3.5 text-cyan-400 shrink-0" />
                    <span>{tier.aiCreditsText}</span>
                  </div>

                  {/* AI Voice Calls */}
                  <div className="bg-purple-900/40 border border-purple-500/25 p-1.5 rounded-xl flex items-center gap-1.5 text-[10px] font-extrabold text-amber-200">
                ...
                <div className="space-y-2 pt-1">
                  ...
                  <div className="space-y-2 text-[11px] font-semibold text-slate-200">
                    <div className="flex items-start gap-1.5">
                      ...
                      <span><strong>ATS Scans:</strong> {tier.limits.path1.atsScans}</span>
                    </div>
                    <div className="flex items-start gap-1.5">
                      <CheckCircle2 className="w-3.5 h-3.5 text-purple-400 shrink-0 mt-0.5" />
                      <span><strong>Mock Interviews:</strong> {tier.limits.path1.mockInterviews}</span>
                    </div>
                    <div className="flex items-start gap-1.5">
                      <CheckCircle2 className="w-3.5 h-3.5 text-purple-400 shrink-0 mt-0.5" />
                      <span><strong>Skill Courses:</strong> {tier.limits.path2.activeCourses}</span>
                    </div>
                    <div className="flex items-start gap-1.5">
                      <CheckCircle2 className="w-3.5 h-3.5 text-purple-400 shrink-0 mt-0.5" />
                      <span><strong>MSME Filings:</strong> {tier.limits.path3.msmeFilings}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-purple-900/40 mt-4 space-y-2">
                ...
                <span className="text-[8px] text-slate-400 font-bold block text-center uppercase tracking-wider">
      ...
      <div className="bg-[#120a28] border border-purple-900/60 p-6 md:p-8 rounded-[2.5rem] shadow-2xl space-y-6 text-left">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
        <div className="bg-[#120b29] border border-purple-900/50 p-5 rounded-2xl space-y-2">
